...
| 9 — spatie RBAC | Hecha: tablas `spatie_*` (prefijo, las legacy `roles`/`permisos` no se tocan), `AppServicesegacyrbacsync`, comando `php artisan rbac:sync [--verify]` (286 comprobaciones usuario � permiso id�nticas a `rol_permiso`), sincronizaci�n del usuario en cada login. backup previo: `backups/backup_db_2026-09-13_20-03_pre_tarea9_spatie.sql`. |
