...
// "critico" bloquea programar viajes con ese bus; "aviso" es solo prolijidad
// de datos (no impide vender). Se separan para no mezclar urgencias.
$revisionCritica = [];
$revisionAviso = [];
foreach ($data['vehiculos'] ?? [] as $bus) {
    if (empty($bus->tipo_bus_id)) {
        $revisionCritica[] = ['id' => $bus->id, 'placa' => $bus->placa, 'texto' => "Sin tipo de bus ({$bus->asientos} asientos registrados): no se pueden programar viajes con él."];
    } elseif ((int) $bus->tipo_capacidad !== (int) $bus->asientos) {
        $revisionAviso[] = ['id' => $bus->id, 'placa' => $bus->placa, 'texto' => "El tipo {$bus->tipo_nombre} tiene {$bus->tipo_capacidad} asientos pero el bus figura con {$bus->asientos}."];
    }
                    ...
                    <div class="card shadow-sm border-0">
                        ...
                        <div class="card-body">
                            ...
                            <?php if ($revisionFlota): ?>
                                <div class="alert alert-warning revision-flota mb-4">
                                    <button type="button" class="revision-flota-toggle" data-bs-toggle="collapse" data-bs-target="#detalleRevisionFlota" aria-expanded="false">
                                        <i class="bi bi-clipboard-check me-2"></i>
                                        <span class="flex-fill text-start">
                                            <?php if ($revisionCritica): ?><strong><?php echo count($revisionCritica); ?> bus(es) no se pueden programar todavía</strong><?php endif; ?>
                                            <?php if ($revisionCritica && $revisionAviso): ?> · <?php endif; ?>
                                            <?php if ($revisionAviso): ?><?php echo count($revisionAviso); ?> dato(s) para prolijar<?php endif; ?>
                                        </span>
                                        <i class="bi bi-chevron-down revision-flota-caret"></i>
                                    </button>
                                    <div class="collapse" id="detalleRevisionFlota">
                                        <div class="revision-flota-lista mt-2">
                                            <?php foreach ($revisionCritica as $item): ?>
                                                <div class="revision-flota-fila critico">
                                                    <span class="badge-placa"><?php echo htmlspecialchars($item['placa']); ?></span>
                                                    <span class="revision-flota-texto"><?php echo htmlspecialchars($item['texto']); ?></span>
                                                    <button type="button" class="btn btn-sm btn-primary" data-id="<?php echo (int) $item['id']; ?>" onclick="abrirModalEditar(this)">Editar</button>
                                                </div>
                                            <?php endforeach; ?>
                                            <?php foreach ($revisionAviso as $item): ?>
                                                <div class="revision-flota-fila">
                                                    <span class="badge-placa"><?php echo htmlspecialchars($item['placa']); ?></span>
                                                    <span class="revision-flota-texto"><?php echo htmlspecialchars($item['texto']); ?></span>
                                                    <button type="button" class="btn btn-sm btn-outline-secondary" data-id="<?php echo (int) $item['id']; ?>" onclick="abrirModalEditar(this)">Editar</button>
                                                </div>
                                            <?php endforeach; ?>
                                        </div>
                                    </div>
                            ...
                            <div class="row mb-4 g-3">
                                <div class="col-md-4">
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control border-start-0 bg-light" id="buscadorFlota" placeholder="Buscar por placa, marca o propietario...">
                                    </div>
                                </div>
                                <div class="col-md-2 ms-auto">
                                    <select class="form-select text-secondary" id="limiteFlota">
                            ...
                            <div class="small text-muted mb-2 d-none" id="sinResultadosFlota"><i class="bi bi-info-circle me-1"></i>Ningún bus coincide con la búsqueda.</div>

                            <!-- Tabla moderna -->
                            <div class="table-responsive">
                                <table class="table table-hover align-middle" id="tablaFlota">
                                    ...
                                    <tbody>
                                        <?php if (!empty($data['vehiculos'])): ?>
                                            <?php foreach ($data['vehiculos'] as $bus): ?>
                                                <tr data-buscar="<?php echo strtolower($bus->placa . ' ' . $bus->marca . ' ' . $bus->modelo . ' ' . $bus->propietario_nombres . ' ' . $bus->propietario_apellidos); ?>">
                                                    <td>
        ...
        <div class="modal-content border-0 rounded-4 shadow-lg">
            ...
            <div class="modal-body p-4">
                <form id="formEditarBus">
                    ...
                    <!-- Sección 1+2: Identidad, legalidad y ficha técnica básica (colapsado: rara vez cambia al editar) -->
                    <div class="accordion mb-4" id="accordionEditIdentidad">
                        <div class="accordion-item border-0 shadow-sm rounded-3">
                            <h2 class="accordion-header">
                                <button class="accordion-button collapsed fw-bold text-secondary bg-light rounded-3" type="button" data-bs-toggle="collapse" data-bs-target="#collapseEditIdentidad">
                                    <i class="bi bi-shield-lock me-2"></i> Identidad, legalidad y ficha técnica
                                </button>
                            </h2>
                            <div id="collapseEditIdentidad" class="accordion-collapse collapse" data-bs-parent="#accordionEditIdentidad">
                                <div class="accordion-body bg-light rounded-bottom-3 mt-1">
                                    ...
                                    <div class="row g-2 mb-3">
                                        <div class="col-md-4">
                                            <label class="form-label text-secondary small fw-bold">Marca</label>
                                            <input type="text" class="form-control form-control-sm" id="editMarca" name="marca" list="marcasList">
                                        </div>
                                        <div class="col-md-4">
                                            <label class="form-label text-secondary small fw-bold">Modelo</label>
                                            <input type="text" class="form-control form-control-sm" id="editModelo" name="modelo">
                                        </div>
                                        <div class="col-md-4">
                                            <label class="form-label text-secondary small fw-bold">Año Fabricación</label>
                                            <input type="number" class="form-control form-control-sm" id="editAnio" name="anio" min="1990" max="2026">
                                        </div>
                                        <div class="col-md-4">
                                            <label class="form-label text-secondary small fw-bold">Clase</label>
                                            <select class="form-select form-select-sm" id="editClase" name="clase">
                                                <option value="Bus">Bus</option>
                                                <option value="Minivan">Minivan</option>
                                                <option value="Vagoneta">Vagoneta</option>
                                            </select>
                                        ...
                                        <div class="col-md-4">
                                            <label class="form-label text-secondary small fw-bold">Carrocería</label>
                                            <input type="text" class="form-control form-control-sm" id="editCarroceria" name="carroceria">
                                        </div>
                                        <div class="col-md-2">
                                            <label class="form-label text-secondary small fw-bold">Ejes</label>
                                            <input type="number" class="form-control form-control-sm" id="editEjes" name="ejes">
                                        </div>
                                        <div class="col-md-2">
                                            <label class="form-label text-secondary small fw-bold">Ruedas</label>
                                            <input type="number" class="form-control form-control-sm" id="editRuedas" name="ruedas">
                                        </div>
                                    </div>
                                    <h6 class="text-uppercase small text-muted fw-bold mb-2">Motor y Serie (opcional)</h6>
                                    <div class="row g-2 mb-3">
                                        <div class="col-md-6">
                                    ...
                                    <h6 class="text-uppercase small text-muted fw-bold mb-2">Dimensiones (metros) y Pesos (kg) (opcional)</h6>
                                    <div class="row g-2">
                                        <div class="col-md-2">
...
<style>
    /* Revision de flota: colapsada por defecto, un resumen corto arriba y
       el detalle (con boton Editar por bus) solo si se abre. */
    .revision-flota { padding: 0; overflow: hidden; }
    .revision-flota-toggle {
        display: flex; align-items: center; width: 100%; border: 0; background: none;
        padding: .85rem 1rem; font-weight: 600; color: inherit; cursor: pointer;
    }
    .revision-flota-caret { transition: transform .2s ease; }
    .revision-flota-toggle[aria-expanded="true"] .revision-flota-caret { transform: rotate(180deg); }
    .revision-flota-lista { padding: 0 1rem 1rem; display: flex; flex-direction: column; gap: .5rem; }
    .revision-flota-fila { display: flex; align-items: center; gap: .75rem; background: rgba(255, 255, 255, .5); border-radius: 10px; padding: .5rem .75rem; }
    .revision-flota-fila.critico { background: rgba(255, 255, 255, .8); border: 1px solid rgba(220, 38, 38, .35); }
    .badge-placa { font-family: monospace; font-weight: 800; background: #111827; color: #fff; padding: .15rem .5rem; border-radius: 6px; flex-shrink: 0; }
    .revision-flota-texto { flex: 1; font-size: .85rem; }
    body.dark-mode .revision-flota-fila { background: rgba(0, 0, 0, .15); }
    body.dark-mode .revision-flota-fila.critico { background: rgba(0, 0, 0, .25); border-color: rgba(248, 113, 113, .4); }
...
<script>
    // ---------- Búsqueda y límite de filas (solo front-end, no toca el servidor) ----------
    (function() {
        const buscador = document.getElementById('buscadorFlota');
        const limite = document.getElementById('limiteFlota');
        const filas = [...document.querySelectorAll('#tablaFlota tbody tr[data-buscar]')];
        const sinResultados = document.getElementById('sinResultadosFlota');
        if (!buscador || !filas.length) return;

        function aplicar() {
            const texto = buscador.value.trim().toLowerCase();
            const max = parseInt(limite.value, 10) || 0;
            let visibles = 0;
            filas.forEach(fila => {
                const coincide = !texto || fila.dataset.buscar.includes(texto);
                const dentroDelLimite = !max || visibles < max;
                const mostrar = coincide && dentroDelLimite;
                fila.classList.toggle('d-none', !mostrar);
                if (coincide) visibles++;
            });
            const totalCoincidencias = filas.filter(f => !texto || f.dataset.buscar.includes(texto)).length;
            sinResultados.classList.toggle('d-none', totalCoincidencias !== 0);
        }
        buscador.addEventListener('input', aplicar);
        limite.addEventListener('change', aplicar);
        aplicar();
    })();
